...
- Una fila de salida por cada fila de detalle. Total de filas = HC + PC + BnW + NT.
- `BU` guarda el nombre de la hoja de origen; `ID_Detalle` el `ID` propio de esa hoja hija.
- Las encuestas **sin ninguna fila de detalle se omiten**.
...
|---|---|---|---|---|
| `incremental` | Sí | Sí, en el sitio | No | Varias veces al día |
| `total` | Reconstruye | Reconstruye | Sí | Una vez al día |

La llave de identidad de una fila es `BU + ID_Detalle`.
...
1. **Service account.** En la consola de GCP, crear (o reutilizar) una service account y generar una llave JSON. No necesita ningún rol de IAM: el acceso se otorga compartiendo las planillas.
...
3. **Secret en GitHub.** `Settings → Secrets and variables → Actions → New repository secret`, nombre `GCP_SA_KEY`, y pegar el JSON completo de la llave.
4. **Primera corrida.** Ejecutar manualmente en `Actions → Transformar Encuesta Especial → Run workflow`, con modo `total` y `dry_run` activado para revisar el log. Si los conteos cuadran, repetir sin `dry_run`.
...
- **URL** (ambos): `https://api.github.com/repos/USUARIO/REPO/dispatches`
- **Método**: `POST`
- **Headers**:
  ```
